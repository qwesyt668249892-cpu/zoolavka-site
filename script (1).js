...
/* Telegram-бот для приёма заказов владельцем (клиент его не видит).
   Заказ уходит владельцу в Telegram, а связывается она с клиентом сама,
   тем мессенджером, который клиент выбрал в форме. */
const TG_BOT_TOKEN = "8713647672:AAG7TLFs3OGaZc7gqZWQIsmFzac8axNUk18";
const TG_CHAT_ID   = "958072190";

/* ВИДЕО В ШАПКЕ. Вставь сюда прямую ссылку на видео (.mp4 или .webm).
   Пусто ("") = остаётся анимация с сыплющейся пачкой.
   Пример: const HERO_VIDEO = "https://site.ru/pour.mp4"; */
const HERO_VIDEO = "";

/* ================= ТОВАРЫ И ЦЕНЫ — редактируй здесь =================
   price для товара без вариантов; variants:[{l:"метка", p:цена}] для товаров с выбором.
   img — ключ фото. gallery — доп. расцветки (для пелёнок).                          */
const CATS = [
...
/* Картинки разделов: вставь сюда base64 (data:image/...) или URL. Ключ = id раздела.
   Пусто = показывается заглушка с лапкой. */
...
let activeCat = "", query = "";
const cart = {}; // key -> {id,name,vl,price,img,qty}

/* ---- category selection (навигация по плиткам) ---- */
...
(function runDog(){
  ...
  if(!host||!hero) return;
  if(HERO_VIDEO){                       // пользователь вставил своё видео
    ...
    host.appendChild(v);
    v.play&&v.play().catch(()=>{});
    return;                             // без бегающего пёсика, играет видео
  }
  ...
  let x=0, dir=1, fIdx=0; const SPEED=52, FPS=11; // px/с и кадров/с бега
  ...
  // Стартуем анимацию только после полной загрузки страницы (картинки, шрифты) —
  // иначе на медленном первом открытии страница ещё занята загрузкой ресурсов,
  // и бег получается рывками. После полной загрузки (и на повторных заходах,
  // когда всё уже в кэше) собачка бежит сразу и плавно.
